...
class DeviceFactory:
    ...
    def get_ui_hierarchy_xml(self, device_id: str | None = None) -> str | None:
        """
        Get UI hierarchy XML for current screen (if supported by device backend).

        For ADB devices this uses `uiautomator dump`. For other backends it may be
        unavailable and return None.
        """
        getter = getattr(self.module, "get_ui_hierarchy_xml", None)
        if callable(getter):
